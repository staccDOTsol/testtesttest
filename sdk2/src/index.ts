...
const MPL_TM_BUF = (new PublicKey("metaqbxxUerdq28cj1RbAWkYQm3ybzjb6a8bt518x1s")).toBuffer();
...
export class FanoutClient {
  ...
  constructor(connection: Connection, wallet: Keypair) {
    ...
    this.wallet = wallet;
    this.provider = new AnchorProvider(connection, new Wallet(wallet), {});
  }
      ...
      members.map(async (member) => {
        switch (fanoutAcct.membershipModel) {
          ...
          case MembershipModel.NFT:
            ...
            const wallet = (await getTokenAccount(this.provider as unknown as Provider, account))
              .owner;
